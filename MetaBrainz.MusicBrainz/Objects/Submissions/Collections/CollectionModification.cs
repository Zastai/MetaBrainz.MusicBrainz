...
[PublicAPI]
public sealed class CollectionModification : CollectionModification<CollectionModification, IEntity> {
...
/// <summary>A submission request for adding or removing items to a collection.</summary>
/// <typeparam name="T">The specific type of collection modification request.</typeparam>
/// <typeparam name="TEntity">The type of entity the collection holds.</typeparam>
[PublicAPI]
public abstract class CollectionModification<T, TEntity> where T : CollectionModification<T, TEntity> where TEntity : IEntity {
  ...
  /// <returns>This collection modification helper.</returns>
  public T Include(params IEnumerable<TEntity> items) {
  ...
  /// <param name="items">The items to include in the request.</param>
  /// <returns>This collection modification helper.</returns>
  public T Include(params ReadOnlySpan<TEntity> items) {
    foreach (var item in items) {
      this.IncludeItem(item);
    }
    return (T) this;
  ...
  /// <param name="items">The items to include in the request.</param>
  /// <returns>This collection modification helper.</returns>
  public async Task<T> IncludeAsync(IAsyncEnumerable<TEntity> items) {
  ...
  internal CollectionModification(Query query, string client, Guid id, EntityType type) {
  ...
  private void IncludeItem(TEntity item) {
